...
parser.add_argument('--benchDir', help="Path to Benchmark Directory Relative to M5_PATH", required=True)
parser.add_argument('--path', help="Path to M5 Directory", required=False)
parser.add_argument('--output-dir', dest='output_dir', help="Alternate output directory", required=False)
parser.add_argument('--dry-run', dest='dry_run', action='store_true', help="Preview without writing files")
parser.add_argument('--validate-only', dest='validate_only', action='store_true', help="Compare against existing files")
parser.add_argument('--base-address', dest='base_address', type=lambda x: int(x, 0),
                    default=0x10020000, help="Base address for accelerator cluster (default: 0x10020000)")
...
def main():
    global temp_dir
    baseAddress = args.base_address  # Configurable via --base-address (default: 0x10020000)
...
def genConfigFile(clusters):
    ...
    for i in clusters:
        ...
        content.append("\tbuild" + i.name + "(args, system, system." + i.name.lower() + ")\n\n")

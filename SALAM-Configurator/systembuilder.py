...
parser.add_argument(
    "--sysName", help="Desired gem5 script name", required=True
)
parser.add_argument(
    "--benchDir",
    help="Path to Benchmark Directory Relative to M5_PATH",
    required=True,
)
...
def write_file(path, content, description=""):
    if args.dry_run:
        ...
        with open(path, "w") as f:
            f.write(content) if isinstance(content, str) else f.writelines(
                content
            )
        print(f"Wrote: {path}")
...
def parseYAMLFile(config, configPath, baseAddress):
    ...
    for clusterList in config:
        ...
        clusters.append(
            AccCluster(
                clusterName, dmas, accs, baseAddress, M5_Path, configPath
            )
        )
...
def genConfigFile(clusters):
    ...
    content.append(
        "# " + fileName + ".py - Auto-Generated Accelerator Configuration\n"
    )
    content.append(
        "# ==============================================================================\n"
    )
    ...
    content.append(
        "WARNING: This file is auto-generated by systembuilder.py.\n"
    )
    content.append(
...
def genHeaderFiles(headerList, clusters):
    ...
    for idx, cluster in enumerate(clusters):
        ...
        write_file(
            output_path, "".join(currentHeader), f"header for {cluster.name}"
        )
...
def genFullSystemFile(clusters):
    ...
    f.close()
    fullSystem[68] = "import " + fileName + "\n"
    fullSystem[250] = "        " + fileName + ".makeHWAcc(args, test_sys)\n"

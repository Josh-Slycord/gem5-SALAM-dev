                    ...
                    if "InterruptNum" in deviceDict:
                        dmaClass.append(
                            Dma(
                                ...
                                topAddress,
                                deviceDict["Type"],
                                deviceDict["InterruptNum"],
                                deviceDict["BufferSize"],
                                deviceDict["MaxReqSize"],
                            )
                        )
                    else:
                        dmaClass.append(
        ...
        for acc in self.accs:
            ...
            for deviceDict in acc["Accelerator"]:
                if "PIOSize" in deviceDict:
                    ...
                    pioSize = deviceDict["PIOSize"] + (
                        64 - (deviceDict["PIOSize"] % 64)
                    )
    ...
    def genConfig(self):
        ...
        for master in self.pioMasters:
            if "LocalBus" in master:
                ...
                lines.append(
                    "clstr." + self.name + ".pio " + "=" " clstr." + i + ".local"
                )
        # Add StreamIn
...
class Dma:
    def __init__(
        self, name, pio, pioMasters, address, dmaType, int_num=None, size=64, maxReq=4
    ):
        self.name = name.lower()
        self.pio = pio
        self.pioMasters = pioMasters

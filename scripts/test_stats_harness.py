...
def test_parser():
    ...
    checks = [
        ...
        ("accelerator_name", stats.performance.accelerator_name == "test_accelerator"),
    ...
    print(f"  stall_percentages = {list(stats.stall_breakdown.get_stall_percentages().keys())[:3]}...")
...
def test_json_schema():
    ...
    stats_keys = [
        "version", "timestamp", "accelerator_name",
        "performance", "memory_access", "dataflow",
        "stall_breakdown", "power", "area"
    ]
...
def main():
    ...
    parser.add_argument(
        ...
        help="Run ZeroMQ stats emulator"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-sample", type=str,
        help="Save sample JSON to file"
    )
    parser.add_argument(
    ...
    if not any([args.parser, args.schema, args.emulator, args.save_sample, args.all]):
        args.all = True

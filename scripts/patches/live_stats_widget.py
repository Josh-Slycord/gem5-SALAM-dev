...
class MetricCard(QFrame):
    ...
    def __init__(
        self, label: str, value: str = "-", color: Optional[QColor] = None
    ):
        super().__init__()
...
class LiveStatsWidget(QWidget):
    ...
    def disconnect_from_simulation(self):
        ...
        if self.connection:
            try:
                self.connection.stats_updated.disconnect(
    ...
    def _update_display(self, stats: EnhancedSimulationStats):
        ...
        stall_pct = (
            (perf.stall_cycles / perf.total_cycles * 100)
        ...
        color = (
            QColor("#27ae60")
            if stall_pct < 20
            else (QColor("#f39c12") if stall_pct < 50 else QColor("#e74c3c"))
        )
        ...
        ipc = (
            perf.executed_nodes / perf.total_cycles
            if perf.total_cycles > 0
            else 0
        )
        ...
        self.total_area_card.set_value(
            format_metric(area.total_area_um2, "um2")
        )

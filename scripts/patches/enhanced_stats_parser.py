...
class EnhancedSimulationStats:
    ...
    version: str = "3.0"
    timestamp: str = ""
    performance: EnhancedPerformanceMetrics = field(
        default_factory=EnhancedPerformanceMetrics
    )
    memory_access: MemoryAccessMetrics = field(
        default_factory=MemoryAccessMetrics
    )
    dataflow: DataflowMetrics = field(default_factory=DataflowMetrics)
    fu_utilization: FUUtilizationMetrics = field(
        default_factory=FUUtilizationMetrics
    )
    stall_breakdown: StallBreakdownMetrics = field(
        default_factory=StallBreakdownMetrics
    )
...
def _parse_stats_dict(data: Dict[str, Any]) -> EnhancedSimulationStats:
    ...
    if "performance" in data:
        ...
        stats.performance = EnhancedPerformanceMetrics(
            ...
            total_time_ns=perf.get("setup_time_ns", 0)
            + perf.get("sim_time_ns", 0),
            clock_period_ns=perf.get("clock_period_ns", 0),
            total_cycles=perf.get("total_cycles", 0),
            stall_cycles=perf.get("stall_cycles", 0),
            active_cycles=perf.get("total_cycles", 0)
            - perf.get("stall_cycles", 0),

...
class SchemaValidator:
    ...
    DMA_OPTIONAL_FIELDS = {'Type', 'MaxReqSize', 'BufferSize', 'WrReqSize',
                           'RdReqSize', 'Flags', 'Debug'}
    ...
    VALID_INSTRUCTIONS = {
        ...
        'icmp', 'fcmp',
        'load', 'store', 'gep', 'alloca', 'fence',
        'sext', 'zext', 'trunc', 'fpext', 'fptrunc',
        'fptosi', 'fptoui', 'sitofp', 'uitofp',
        'inttoptr', 'ptrtoint', 'bitcast', 'addrspacecast',
        'br', 'indirectbr', 'switch', 'call', 'invoke', 'ret',
        'resume', 'unreachable', 'phi', 'select', 'vaarg', 'landingpad'
    }
    ...
    def validate(self, config: Dict[str, Any], config_path: Optional[str] = None) -> ValidationResult:
        """
    ...
    def _validate_acc_cluster(self, acc_cluster: Any) -> None:
        ...
        for i, item in enumerate(acc_cluster):
            if not isinstance(item, dict):
                self.errors.append(f"acc_cluster[{i}]: Expected dict, got {type(item).__name__}")
                continue
            ...
            if 'Accelerator' in item:
                acc_count += len(item['Accelerator']) if isinstance(item['Accelerator'], list) else 1
                self._validate_accelerator_list(item['Accelerator'], i)

        if not has_name:
            self.warnings.append("acc_cluster: Missing 'Name' field (cluster will be unnamed)")
        ...
        logger.debug(f"Validated acc_cluster: {dma_count} DMAs, {acc_count} accelerators")
    ...
    def _validate_accelerator_list(self, acc_list: Any, parent_idx: int) -> None:
        ...
        for i, item in enumerate(acc_list):
            ...
            if not isinstance(item, dict):
                self.errors.append(f"{prefix}: Expected dict, got {type(item).__name__}")
                continue
            ...
            if 'Name' in item:
                ...
                # Check for unknown fields
                known_fields = self.ACCELERATOR_REQUIRED_FIELDS | self.ACCELERATOR_OPTIONAL_FIELDS
    ...
    def _validate_hw_config(self, hw_config: Any) -> None:
        ...
        for bench_name, bench_config in hw_config.items():
            ...
            if 'functional_units' in bench_config:
                self._validate_functional_units(bench_config['functional_units'], bench_name)

    def _validate_instructions(self, instructions: Any, bench_name: str) -> None:
        """Validate instruction configurations."""
        if not isinstance(instructions, dict):
            self.errors.append(f"hw_config.{bench_name}.instructions: Must be a dictionary")
            return
        ...
        for inst_name, inst_config in instructions.items():
            ...
            if 'runtime_cycles' in inst_config:
                ...
                if not isinstance(cycles, (int, float)) or cycles < 0:
                    self.errors.append(f"{prefix}.runtime_cycles: Must be a non-negative number")
    ...
    def _validate_functional_units(self, fus: Any, bench_name: str) -> None:
        ...
        if not isinstance(fus, dict):
            self.errors.append(f"hw_config.{bench_name}.functional_units: Must be a dictionary")
            return
        ...
        for fu_name, fu_config in fus.items():
            ...
            if 'cycles' in fu_config:
                ...
                if not isinstance(cycles, (int, float)) or cycles < 0:
                    self.errors.append(f"{prefix}.cycles: Must be a non-negative number")

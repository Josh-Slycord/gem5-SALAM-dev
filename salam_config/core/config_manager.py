...
class ConfigManager:
    ...
    def load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValidationError(f"YAML parsing error: {e}", file_path=str(path))
    ...
    def parse_cluster_config(self, config: Dict[str, Any]) -> ClusterConfig:
        ...
        logger.debug(
            f"Parsed cluster: {cluster_name} with {len(dmas)} DMAs, {len(accelerators)} accelerators"
        )
    ...
    def generate_all(
        ...
        if not self._m5_path:
            return GenerationResult(
                success=False, error="M5_PATH environment variable not set"
            )

        bench_path = Path(self._m5_path) / bench_dir / benchmark
        if not bench_path.exists():
            return GenerationResult(
                success=False, error=f"Benchmark directory not found: {bench_path}"
            )
        ...
        if not config_file.exists():
            ...
            for alt_name in ["config.yaml", f"{benchmark}_config.yml"]:
                ...
                return GenerationResult(
                    success=False, error=f"Configuration file not found in {bench_path}"
                )
        ...
        # NOTE: C header generation disabled - SALAM-Configurator handles this
        # The *_clstr_hw_defines.h files are generated by systembuilder.py
        # and placed at the benchmark root (not hw/ subdirectory)
        # Enabling this would create orphan files that conflict with the build
        #
        # try:
        ...
        return GenerationResult(
            success=True, generated_files=generated_files, warnings=warnings
        )
    ...
    def _render_python_config(
        self, benchmark: str, cluster: ClusterConfig, config: Dict[str, Any]
    ) -> str:
        ...
        for dma in cluster.dmas:
            lines.append(
                f'    {{"name": "{dma.name}", "type": "{dma.dma_type}", '
        ...
        for acc in cluster.accelerators:
            lines.append(
                f'    {{"name": "{acc.name}", "ir_path": "{acc.ir_path}", '
    ...
    def _render_c_header(
        self, benchmark: str, cluster: ClusterConfig, base_address: int
    ) -> str:
    ...
    def _generate_hw_profile(
        self, benchmark: str, bench_path: Path, cycle_time: str, dry_run: bool
    ) -> List[str]:
    ...
    def get_benchmark_info(
        self, benchmark: str, bench_dir: str = "benchmarks/sys_validation"
    ) -> Dict[str, Any]:
        """
